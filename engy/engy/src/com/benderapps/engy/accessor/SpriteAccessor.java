package com.benderapps.engy.accessor;
...
public class SpriteAccessor implements TweenAccessor<Sprite> {
	...
	public int getValues(Sprite target, int tweenType, float[] returnValues) {
		switch (tweenType) {
		case FLIP:
			returnValues[0] = target.isFlipX() ? 1 : 0;
			returnValues[1] = target.isFlipY() ? 1 : 0;
			return 2;
		case POS_XY:
	...
	public void setValues(Sprite target, int tweenType, float[] newValues) {
		switch (tweenType) {
		case FLIP:
			target.flip(newValues[0] == 1 ? true : false,
					newValues[1] == 1 ? true : false);
			break;
		case POS_XY:
